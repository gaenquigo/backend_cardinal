...
/**
 * Class EvidencesSourcesController
 * @package App\Http\Controllers
 */
class EvidencesSourcesController extends UtilsController
{
    /**
     * Funcion para obtener los recursos que se le
     * asociaron a una evidencia
     */
    public function index(Request $request)
    {
        try {
            $validator = $this->validateEvidenceSourceRequest($request);
            $errors = true;
            $response_info = [];
                                ...
                                if ($activity_source) {
                                    ...
                                    if ($evidence_financial) {
                                        ...
                                        if ($request["value"] > $activity_source->evidence_balance) {
                                            ...
                                            $response_info["message"] = "El recurso asociado a la evidencia se creó correctamente";
                                            $activity_source->save();
                                            $errors = false;
                                        }
                                    }
                            ...
                            if ($financial_plan) {
                                ...
                                if ($activity_source) {
                                    ...
                                    if ($evidence_financial) {
                                        ...
                                        $activity_source->save();
                                        $response_info["message"] = "El recurso asociado a la evidencia se eliminó correctamente";
     ...
     * obtener, asociar y deasociar recursos a la evidencia
     */
    private function validateEvidenceSourceRequest($request)
    {
        $rules = [];
        $messages = [];
        $attributes = [];
