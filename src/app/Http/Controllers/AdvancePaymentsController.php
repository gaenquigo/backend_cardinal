 ...
 */
class AdvancePaymentsController extends UtilsController
{
    /**
    ...
    {
        try {
            $validator = $this->validateAdvancePaymentRequest($request);
            $errors = true;
            ...
            if ($validator->fails()) {
                ...
                $responseValidation = UtilsController::checkStructure($request, "advance", true, true);
                $errors = $responseValidation["errors"];
                if (!$responseValidation["errors"]) {
                    $advance = $responseValidation["data"];
                    $payments = Payment::where('evidence_id', $advance["id"])->get();
                    $response_info = $advance["payments"] = $payments;
                    $errors = false;
                } else {
    ...
    public function store(Request $request)
    {
        try {
            ...
            if ($validator->fails()) {
                ...
                if (!$responseValidation["errors"]) {
                    $advance = $responseValidation["data"];
                    $evidence_financial = EvidenceFinancial::where('evidence_id', $advance["id"])->sum('value');
                    $summatory_payment = Payment::where('evidence_id', $advance["id"])->sum('value');
                    ...
                    if ($summatory_payment > $evidence_financial) {
                        ...
                        foreach ($request["payments"]  as $keyPayment =>  $payment) {
                            if ($expensesController->checkYearDate($request["year"], $pai, $payment["date"], $advance->date_report)) {
                                $arrayPayment = [
                                    'value' => $payment["value"],
        ...
        try {
            ...
            if ($validator->fails()) {
                ...
                if (!$responseValidation["errors"]) {
                    ...
                    $response_info = "Se ha eliminado correctamente los pagos del avance";
                    $errors = false;
                    FacadesDB::commit();
                } else {
                    $response_info->errors = $responseValidation["message"];
     ...
     * eliminacion y obtencion de los pagos del avance
     * de una evidencia
     */
    private function validateAdvancePaymentRequest($request)
    {
        $rules = [];
        $messages = [];
        $attributes = [];
