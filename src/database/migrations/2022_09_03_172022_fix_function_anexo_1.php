...
class FixFunctionAnexo1 extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        ...
        cur_codes CURSOR FOR
            SELECT DISTINCT 
                    ...
                    SUM(COALESCE(avanceoblig.av_oblig, 0)) OVER (PARTITION BY p.id) AS pai_av_oblig,
                    SUM(COALESCE(avancepagos.av_pagos, 0)) OVER (PARTITION BY p.id) AS pai_av_pagos,
                    ...
                    SUM(COALESCE(fnacum.acum_av_fin, 0)) OVER (PARTITION BY p.id) AS pai_av_fin_acum,
                    ...
                    SUM(COALESCE(actsc.met_fin_anual, 0)) OVER (PARTITION BY strln.id) AS strln_met_fin,
                    SUM(COALESCE(avancefn.av_met_fin, 0)) OVER (PARTITION BY strln.id) AS strln_av_fin,
                    SUM(COALESCE(avanceoblig.av_oblig, 0)) OVER (PARTITION BY strln.id) AS strln_av_oblig,
                    SUM(COALESCE(avancepagos.av_pagos, 0)) OVER (PARTITION BY strln.id) AS strln_av_pagos,
                    SUM(COALESCE(actscttal.met_fin_total, 0)) OVER (PARTITION BY strln.id) AS strln_met_fin_total,
                    SUM(COALESCE(actscacum.met_fin_acum, 0)) OVER (PARTITION BY strln.id) AS strln_met_fin_acum,
                    SUM(COALESCE(fnacum.acum_av_fin, 0)) OVER (PARTITION BY strln.id) AS strln_av_fin_acum,
                    ...
                    SUM(COALESCE(avanceoblig.av_oblig, 0)) OVER (PARTITION BY prog.id) AS prog_av_oblig,
                    SUM(COALESCE(avancepagos.av_pagos, 0)) OVER (PARTITION BY prog.id) AS prog_av_pagos,
                    ...
                    SUM(COALESCE(fnacum.acum_av_fin, 0)) OVER (PARTITION BY prog.id) AS prog_av_fin_acum,
                    ...
                    SUM(COALESCE(avanceoblig.av_oblig, 0)) OVER (PARTITION BY proy.id) AS proy_av_oblig,
                    SUM(COALESCE(avancepagos.av_pagos, 0)) OVER (PARTITION BY proy.id) AS proy_av_pagos,
                    ...
                    SUM(COALESCE(fnacum.acum_av_fin, 0)) OVER (PARTITION BY proy.id) AS proy_av_fin_acum,
                    ...
                    SUM(COALESCE(avanceoblig.av_oblig, 0)) OVER (PARTITION BY obj.id) AS obj_av_oblig,
                    SUM(COALESCE(avancepagos.av_pagos, 0)) OVER (PARTITION BY obj.id) AS obj_av_pagos,
                    ...
                    SUM(COALESCE(fnacum.acum_av_fin, 0)) OVER (PARTITION BY obj.id) AS obj_av_fin_acum,
                    ...
                    objprod.goal_1 AS product_goal_1,
                    objprod.goal_2 AS product_goal_2,
                    objprod.goal_3 AS product_goal_3,
                    objprod.goal_4 AS product_goal_4,
                    ...
                    SUM(COALESCE(avanceoblig.av_oblig, 0)) OVER (PARTITION BY objprod.id) AS product_av_oblig,
                    SUM(COALESCE(avancepagos.av_pagos, 0)) OVER (PARTITION BY objprod.id) AS product_av_pagos,
                    ...
                    SUM(COALESCE(fnacum.acum_av_fin, 0)) OVER (PARTITION BY objprod.id) AS product_av_fin_acum,
                    act.id as act_id, 
            ...
            BEGIN
                ...
                FOR registro IN cur_codes LOOP
                    ...
                    IF id_act_tmp = 0 THEN
                        ...
                        INSERT INTO temp_anexo_1(id_source,id_pai,id_lin,id_prod,id_obj,id_prog,id_proy,level_t,grupo_pai,id,name,ponderacion,indicador,unit_goal,name_unit_goal,
                                                total_goal,goal_1, goal_2,goal_3,goal_4,repor_vigencia_1,repor_rezago_1,desc_vig_1,desc_rez_1,goal_anio,pc_av_fis,pc_av_fis_rez,pc_av_acum,
                        ...
                        VALUES (registro.act_id,registro.id_pai,registro.id_lin,registro.product_id,registro.objective_id,registro.program_id,registro.project_id,
                                ...
                                registro.goal_anio,registro.act_percent_avance,0,0,
                    ...
                    IF id_prod_tmp = 0 THEN
                        ...
                        VALUES (registro.act_id,registro.id_pai,registro.id_lin,registro.product_id,registro.objective_id,registro.program_id,registro.project_id,
                                ...
                                registro.goal_anio,pc_av_fis,0,0,registro.product_met_fin,registro.product_av_fin,registro.product_av_oblig,registro.product_av_pagos,
                    ...
                    IF id_obj_tmp = 0 THEN
                        ...
                        VALUES (registro.act_id,registro.id_pai,registro.id_lin,registro.product_id,registro.objective_id,registro.program_id,registro.project_id,
                                ...
                                0,0,0,0,0,0,0,'NA','NA',0,pc_av_fis,0,0,registro.obj_met_fin,registro.obj_av_fin,registro.obj_av_oblig,registro.obj_av_pagos,
                    ...
                    IF id_proy_tmp = 0 THEN
                        ...
                        VALUES (registro.act_id,registro.id_pai,registro.id_lin,registro.product_id,registro.objective_id,registro.program_id,registro.project_id,
                                ...
                                0,0,0,0,0,0,0,'NA','NA',0,pc_av_fis,0,0,registro.proy_met_fin,registro.proy_av_fin,registro.proy_av_oblig,registro.proy_av_pagos,
                    ...
                    IF id_prog_tmp = 0 THEN
                        ...
                        VALUES (registro.act_id,registro.id_pai,registro.id_lin,registro.product_id,registro.objective_id,registro.program_id,registro.project_id,
                                ...
                                0,0,0,0,0,0,0,'NA','NA',0,pc_av_fis,0,0,registro.prog_met_fin,registro.prog_av_fin,registro.prog_av_oblig,registro.prog_av_pagos,
                    ...
                    IF id_lin_tmp = 0 THEN
                        ...
                        VALUES (registro.act_id,registro.id_pai,registro.id_lin,registro.product_id,registro.objective_id,registro.program_id,registro.project_id,
                                ...
                                0,0,0,0,0,0,0,'NA','NA',0,pc_av_fis,0,0,registro.strln_met_fin,registro.strln_av_fin,registro.strln_av_oblig,registro.strln_av_pagos,
                    ...
                    IF id_pai_tmp = 0 THEN
                        ...
                        VALUES (registro.act_id,registro.id_pai,registro.id_lin,registro.product_id,registro.objective_id,registro.program_id,registro.project_id,
                                ...
                                0,0,0,0,0,0,0,'NA','NA',0,pc_av_fis,0,0,registro.pai_met_fin,registro.pai_av_fin,registro.pai_av_oblig,registro.pai_av_pagos,
                ...
                temp_anexo_1.desc_rez_1,temp_anexo_1.goal_anio,temp_anexo_1.pc_av_fis,temp_anexo_1.pc_av_fis_rez,temp_anexo_1.pc_av_acum,
