<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

class PlanFinancieroCormacarena extends Migration
{
    /**
     * Run the migrations.
     *
     * @return void
     */
    public function up()
    {
        DB::table('financial_plans')->insert([
            'p_a_i_id' => 145,
            'source_id' => 2735,
            'value' => 4053798008,
            'functioning_percentage' => 40.5825214713066,
            'debt_service_percentage' => 0,
            'fund_percentage' => 0,
            'investment_percentage' => 59.4174785286934,
            'functioning' => 1645133447,
            'debt_service' => 0,
            'fund' => 0,
            'investment' => 2408664561,
            'year' => 3,
            ]);
            $idPlanAniotres1 = DB::getPdo()->lastInsertId();
            
        DB::table('financial_plans')->insert([
            'p_a_i_id' => 145,
            'source_id' => 2757,
            'value' => 2757618674,
            'functioning_percentage' => 0,
            'debt_service_percentage' => 0,
            'fund_percentage' => 0,
            'investment_percentage' => 100,
            'functioning' => 0,
            'debt_service' => 0,
            'fund' => 0,
            'investment' => 2757618674,
            'year' => 3,
            ]);
            $idPlanAniotres2 = DB::getPdo()->lastInsertId();
            
        DB::table('financial_plans')->insert([
            'p_a_i_id' => 145,
            'source_id' => 2759,
            'value' => 8425767,
            'functioning_percentage' => 0,
            'debt_service_percentage' => 0,
            'fund_percentage' => 0,
            'investment_percentage' => 100,
            'functioning' => 0,
            'debt_service' => 0,
            'fund' => 0,
            'investment' => 8425767,
            'year' => 3,
            ]);
            $idPlanAniotres3 = DB::getPdo()->lastInsertId();
            
        DB::table('financial_plans')->insert([
            'p_a_i_id' => 145,
            'source_id' => 2764,
            'value' => 361610510,
            'functioning_percentage' => 0,
            'debt_service_percentage' => 0,
            'fund_percentage' => 0,
            'investment_percentage' => 100,
            'functioning' => 0,
            'debt_service' => 0,
            'fund' => 0,
            'investment' => 361610510,
            'year' => 3,
            ]);
            $idPlanAniotres4 = DB::getPdo()->lastInsertId();
            
        DB::table('financial_plans')->insert([
            'p_a_i_id' => 145,
            'source_id' => 2766,
            'value' => 51549426,
            'functioning_percentage' => 0,
            'debt_service_percentage' => 0,
            'fund_percentage' => 0,
            'investment_percentage' => 100,
            'functioning' => 0,
            'debt_service' => 0,
            'fund' => 0,
            'investment' => 51549426,
            'year' => 3,
            ]);
            $idPlanAniotres5 = DB::getPdo()->lastInsertId();
            
        DB::table('financial_plans')->insert([
            'p_a_i_id' => 145,
            'source_id' => 2768,
            'value' => 80285462,
            'functioning_percentage' => 0,
            'debt_service_percentage' => 0,
            'fund_percentage' => 0,
            'investment_percentage' => 100,
            'functioning' => 0,
            'debt_service' => 0,
            'fund' => 0,
            'investment' => 80285462,
            'year' => 3,
            ]);
            $idPlanAniotres6 = DB::getPdo()->lastInsertId();
            
        DB::table('financial_plans')->insert([
            'p_a_i_id' => 145,
            'source_id' => 2770,
            'value' => 2870375237,
            'functioning_percentage' => 0,
            'debt_service_percentage' => 0,
            'fund_percentage' => 0,
            'investment_percentage' => 100,
            'functioning' => 0,
            'debt_service' => 0,
            'fund' => 0,
            'investment' => 2870375237,
            'year' => 3,
            ]);
            $idPlanAniotres7 = DB::getPdo()->lastInsertId();
            
        DB::table('financial_plans')->insert([
            'p_a_i_id' => 145,
            'source_id' => 2784,
            'value' => 1072567768,
            'functioning_percentage' => 75.0621053531417,
            'debt_service_percentage' => 0,
            'fund_percentage' => 0,
            'investment_percentage' => 24.9378946468583,
            'functioning' => 805091948,
            'debt_service' => 0,
            'fund' => 0,
            'investment' => 267475820,
            'year' => 3,
            ]);
            $idPlanAniotres8 = DB::getPdo()->lastInsertId();
            
        DB::table('financial_plans')->insert([
            'p_a_i_id' => 145,
            'source_id' => 2909,
            'value' => 3191617062,
            'functioning_percentage' => 0,
            'debt_service_percentage' => 0,
            'fund_percentage' => 0,
            'investment_percentage' => 100,
            'functioning' => 0,
            'debt_service' => 0,
            'fund' => 0,
            'investment' => 3191617062,
            'year' => 3,
            ]);
            $idPlanAniotres9 = DB::getPdo()->lastInsertId();
            
        DB::table('financial_plans')->insert([
            'p_a_i_id' => 145,
            'source_id' => 2912,
            'value' => 202777368,
            'functioning_percentage' => 0,
            'debt_service_percentage' => 0,
            'fund_percentage' => 0,
            'investment_percentage' => 100,
            'functioning' => 0,
            'debt_service' => 0,
            'fund' => 0,
            'investment' => 202777368,
            'year' => 3,
            ]);
            $idPlanAniotres10 = DB::getPdo()->lastInsertId();
            
        DB::table('financial_plans')->insert([
            'p_a_i_id' => 145,
            'source_id' => 2915,
            'value' => 1561931042,
            'functioning_percentage' => 0,
            'debt_service_percentage' => 0,
            'fund_percentage' => 0,
            'investment_percentage' => 100,
            'functioning' => 0,
            'debt_service' => 0,
            'fund' => 0,
            'investment' => 1561931042,
            'year' => 3,
            ]);
            $idPlanAniotres11 = DB::getPdo()->lastInsertId();
            
        DB::table('financial_plans')->insert([
            'p_a_i_id' => 145,
            'source_id' => 2918,
            'value' => 4409040818,
            'functioning_percentage' => 0,
            'debt_service_percentage' => 0,
            'fund_percentage' => 0,
            'investment_percentage' => 100,
            'functioning' => 0,
            'debt_service' => 0,
            'fund' => 0,
            'investment' => 4409040818,
            'year' => 3,
            ]);
            $idPlanAniotres12 = DB::getPdo()->lastInsertId();
            
        DB::table('financial_plans')->insert([
            'p_a_i_id' => 145,
            'source_id' => 2921,
            'value' => 125000000,
            'functioning_percentage' => 0,
            'debt_service_percentage' => 0,
            'fund_percentage' => 0,
            'investment_percentage' => 100,
            'functioning' => 0,
            'debt_service' => 0,
            'fund' => 0,
            'investment' => 125000000,
            'year' => 3,
            ]);
            $idPlanAniotres13 = DB::getPdo()->lastInsertId();
            
        DB::table('financial_plans')->insert([
            'p_a_i_id' => 145,
            'source_id' => 2926,
            'value' => 898444918,
            'functioning_percentage' => 100,
            'debt_service_percentage' => 0,
            'fund_percentage' => 0,
            'investment_percentage' => 0,
            'functioning' => 898444918,
            'debt_service' => 0,
            'fund' => 0,
            'investment' => 0,
            'year' => 3,
            ]);
            $idPlanAniotres14 = DB::getPdo()->lastInsertId();
            
        DB::table('financial_plans')->insert([
            'p_a_i_id' => 145,
            'source_id' => 2932,
            'value' => 16215192030,
            'functioning_percentage' => 40.5825214824792,
            'debt_service_percentage' => 0,
            'fund_percentage' => 0,
            'investment_percentage' => 59.4174785175208,
            'functioning' => 6580533789,
            'debt_service' => 0,
            'fund' => 0,
            'investment' => 9634658241,
            'year' => 3,
            ]);
            $idPlanAniotres15 = DB::getPdo()->lastInsertId();
            
        DB::table('financial_plans')->insert([
            'p_a_i_id' => 145,
            'source_id' => 2936,
            'value' => 516802251,
            'functioning_percentage' => 100,
            'debt_service_percentage' => 0,
            'fund_percentage' => 0,
            'investment_percentage' => 0,
            'functioning' => 516802251,
            'debt_service' => 0,
            'fund' => 0,
            'investment' => 0,
            'year' => 3,
            ]);
            $idPlanAniotres16 = DB::getPdo()->lastInsertId();
            
        DB::table('financial_plans')->insert([
            'p_a_i_id' => 145,
            'source_id' => 2938,
            'value' => 4748849682.4,
            'functioning_percentage' => 0,
            'debt_service_percentage' => 0,
            'fund_percentage' => 0,
            'investment_percentage' => 100,
            'functioning' => 0,
            'debt_service' => 0,
            'fund' => 0,
            'investment' => 4748849682.4,
            'year' => 3,
            ]);
            $idPlanAniotres17 = DB::getPdo()->lastInsertId();
            
        DB::table('financial_plans')->insert([
            'p_a_i_id' => 145,
            'source_id' => 2939,
            'value' => 73256129.04,
            'functioning_percentage' => 0,
            'debt_service_percentage' => 0,
            'fund_percentage' => 0,
            'investment_percentage' => 100,
            'functioning' => 0,
            'debt_service' => 0,
            'fund' => 0,
            'investment' => 73256129.04,
            'year' => 3,
            ]);
            $idPlanAniotres18 = DB::getPdo()->lastInsertId();
            
        DB::table('financial_plans')->insert([
            'p_a_i_id' => 145,
            'source_id' => 3771,
            'value' => 11548480.35,
            'functioning_percentage' => 0,
            'debt_service_percentage' => 0,
            'fund_percentage' => 0,
            'investment_percentage' => 100,
            'functioning' => 0,
            'debt_service' => 0,
            'fund' => 0,
            'investment' => 11548480.35,
            'year' => 3,
            ]);
            $idPlanAniotres19 = DB::getPdo()->lastInsertId();
            
        DB::table('financial_plans')->insert([
            'p_a_i_id' => 145,
            'source_id' => 3770,
            'value' => 1143835510.93,
            'functioning_percentage' => 0,
            'debt_service_percentage' => 0,
            'fund_percentage' => 0,
            'investment_percentage' => 100,
            'functioning' => 0,
            'debt_service' => 0,
            'fund' => 0,
            'investment' => 1143835510.93,
            'year' => 3,
            ]);
            $idPlanAniotres20 = DB::getPdo()->lastInsertId();
            
        DB::table('financial_plans')->insert([
            'p_a_i_id' => 145,
            'source_id' => 3772,
            'value' => 40586314.12,
            'functioning_percentage' => 0,
            'debt_service_percentage' => 0,
            'fund_percentage' => 0,
            'investment_percentage' => 100,
            'functioning' => 0,
            'debt_service' => 0,
            'fund' => 0,
            'investment' => 40586314.12,
            'year' => 3,
            ]);
            $idPlanAniotres21 = DB::getPdo()->lastInsertId();
            
        DB::table('financial_plans')->insert([
            'p_a_i_id' => 145,
            'source_id' => 3773,
            'value' => 1743614325.5,
            'functioning_percentage' => 0,
            'debt_service_percentage' => 0,
            'fund_percentage' => 0,
            'investment_percentage' => 100,
            'functioning' => 0,
            'debt_service' => 0,
            'fund' => 0,
            'investment' => 1743614325.5,
            'year' => 3,
            ]);
            $idPlanAniotres22 = DB::getPdo()->lastInsertId();
            
        DB::table('financial_plans')->insert([
            'p_a_i_id' => 145,
            'source_id' => 2940,
            'value' => 71984461.21,
            'functioning_percentage' => 0,
            'debt_service_percentage' => 0,
            'fund_percentage' => 0,
            'investment_percentage' => 100,
            'functioning' => 0,
            'debt_service' => 0,
            'fund' => 0,
            'investment' => 71984461.21,
            'year' => 3,
            ]);
            $idPlanAniotres23 = DB::getPdo()->lastInsertId();
            
        DB::table('financial_plans')->insert([
            'p_a_i_id' => 145,
            'source_id' => 2941,
            'value' => 155343315.24,
            'functioning_percentage' => 0,
            'debt_service_percentage' => 0,
            'fund_percentage' => 0,
            'investment_percentage' => 100,
            'functioning' => 0,
            'debt_service' => 0,
            'fund' => 0,
            'investment' => 155343315.24,
            'year' => 3,
            ]);
            $idPlanAniotres24 = DB::getPdo()->lastInsertId();
            
        DB::table('financial_plans')->insert([
            'p_a_i_id' => 145,
            'source_id' => 2942,
            'value' => 2725034373.73,
            'functioning_percentage' => 0,
            'debt_service_percentage' => 0,
            'fund_percentage' => 0,
            'investment_percentage' => 100,
            'functioning' => 0,
            'debt_service' => 0,
            'fund' => 0,
            'investment' => 2725034373.73,
            'year' => 3,
            ]);
            $idPlanAniotres25 = DB::getPdo()->lastInsertId();
            
        DB::table('financial_plans')->insert([
            'p_a_i_id' => 145,
            'source_id' => 2943,
            'value' => 167065159.67,
            'functioning_percentage' => 0,
            'debt_service_percentage' => 0,
            'fund_percentage' => 0,
            'investment_percentage' => 100,
            'functioning' => 0,
            'debt_service' => 0,
            'fund' => 0,
            'investment' => 167065159.67,
            'year' => 3,
            ]);
            $idPlanAniotres26 = DB::getPdo()->lastInsertId();
            
        DB::table('financial_plans')->insert([
            'p_a_i_id' => 145,
            'source_id' => 2945,
            'value' => 157890976.44,
            'functioning_percentage' => 0,
            'debt_service_percentage' => 0,
            'fund_percentage' => 0,
            'investment_percentage' => 100,
            'functioning' => 0,
            'debt_service' => 0,
            'fund' => 0,
            'investment' => 157890976.44,
            'year' => 3,
            ]);
            $idPlanAniotres27 = DB::getPdo()->lastInsertId();
            
        DB::table('financial_plans')->insert([
            'p_a_i_id' => 145,
            'source_id' => 2946,
            'value' => 198644826.35,
            'functioning_percentage' => 0,
            'debt_service_percentage' => 0,
            'fund_percentage' => 0,
            'investment_percentage' => 100,
            'functioning' => 0,
            'debt_service' => 0,
            'fund' => 0,
            'investment' => 198644826.35,
            'year' => 3,
            ]);
            $idPlanAniotres28 = DB::getPdo()->lastInsertId();
            
        DB::table('financial_plans')->insert([
            'p_a_i_id' => 145,
            'source_id' => 2947,
            'value' => 10173482583.18,
            'functioning_percentage' => 58.8695463445512,
            'debt_service_percentage' => 0,
            'fund_percentage' => 0,
            'investment_percentage' => 41.1304536554488,
            'functioning' => 5989083044.16,
            'debt_service' => 0,
            'fund' => 0,
            'investment' => 4184399539.02,
            'year' => 3,
            ]);
            $idPlanAniotres29 = DB::getPdo()->lastInsertId();
            
        DB::table('financial_plans')->insert([
            'p_a_i_id' => 145,
            'source_id' => 2982,
            'value' => 2409600000,
            'functioning_percentage' => 100,
            'debt_service_percentage' => 0,
            'fund_percentage' => 0,
            'investment_percentage' => 0,
            'functioning' => 2409600000,
            'debt_service' => 0,
            'fund' => 0,
            'investment' => 0,
            'year' => 3,
            ]);
            $idPlanAniotres30 = DB::getPdo()->lastInsertId();
            
        DB::table('financial_plans')->insert([
            'p_a_i_id' => 145,
            'source_id' => 2951,
            'value' => 120036000,
            'functioning_percentage' => 100,
            'debt_service_percentage' => 0,
            'fund_percentage' => 0,
            'investment_percentage' => 0,
            'functioning' => 120036000,
            'debt_service' => 0,
            'fund' => 0,
            'investment' => 0,
            'year' => 3,
            ]);
            $idPlanAniotres31 = DB::getPdo()->lastInsertId();
            
        DB::table('financial_plans')->insert([
            'p_a_i_id' => 145,
            'source_id' => 2952,
            'value' => 16284000,
            'functioning_percentage' => 100,
            'debt_service_percentage' => 0,
            'fund_percentage' => 0,
            'investment_percentage' => 0,
            'functioning' => 16284000,
            'debt_service' => 0,
            'fund' => 0,
            'investment' => 0,
            'year' => 3,
            ]);
            $idPlanAniotres32 = DB::getPdo()->lastInsertId();
            
        DB::table('financial_plans')->insert([
            'p_a_i_id' => 145,
            'source_id' => 2956,
            'value' => 645089522,
            'functioning_percentage' => 100,
            'debt_service_percentage' => 0,
            'fund_percentage' => 0,
            'investment_percentage' => 0,
            'functioning' => 645089522,
            'debt_service' => 0,
            'fund' => 0,
            'investment' => 0,
            'year' => 3,
            ]);
            $idPlanAniotres33 = DB::getPdo()->lastInsertId();
            
        DB::table('financial_plans')->insert([
            'p_a_i_id' => 145,
            'source_id' => 2957,
            'value' => 216113688,
            'functioning_percentage' => 100,
            'debt_service_percentage' => 0,
            'fund_percentage' => 0,
            'investment_percentage' => 0,
            'functioning' => 216113688,
            'debt_service' => 0,
            'fund' => 0,
            'investment' => 0,
            'year' => 3,
            ]);
            $idPlanAniotres34 = DB::getPdo()->lastInsertId();
            
        DB::table('financial_plans')->insert([
            'p_a_i_id' => 145,
            'source_id' => 2958,
            'value' => 21243700,
            'functioning_percentage' => 100,
            'debt_service_percentage' => 0,
            'fund_percentage' => 0,
            'investment_percentage' => 0,
            'functioning' => 21243700,
            'debt_service' => 0,
            'fund' => 0,
            'investment' => 0,
            'year' => 3,
            ]);
            $idPlanAniotres35 = DB::getPdo()->lastInsertId();
            
        DB::table('financial_plans')->insert([
            'p_a_i_id' => 145,
            'source_id' => 2959,
            'value' => 1647598157,
            'functioning_percentage' => 0,
            'debt_service_percentage' => 0,
            'fund_percentage' => 0,
            'investment_percentage' => 100,
            'functioning' => 0,
            'debt_service' => 0,
            'fund' => 0,
            'investment' => 1647598157,
            'year' => 3,
            ]);
            $idPlanAniotres36 = DB::getPdo()->lastInsertId();
            
        DB::table('financial_plans')->insert([
            'p_a_i_id' => 145,
            'source_id' => 2961,
            'value' => 768617586,
            'functioning_percentage' => 0,
            'debt_service_percentage' => 0,
            'fund_percentage' => 0,
            'investment_percentage' => 100,
            'functioning' => 0,
            'debt_service' => 0,
            'fund' => 0,
            'investment' => 768617586,
            'year' => 3,
            ]);
            $idPlanAniotres37 = DB::getPdo()->lastInsertId();
            
        DB::table('financial_plans')->insert([
            'p_a_i_id' => 145,
            'source_id' => 3765,
            'value' => 76780491,
            'functioning_percentage' => 0,
            'debt_service_percentage' => 0,
            'fund_percentage' => 0,
            'investment_percentage' => 100,
            'functioning' => 0,
            'debt_service' => 0,
            'fund' => 0,
            'investment' => 76780491,
            'year' => 3,
            ]);
            $idPlanAniotres38 = DB::getPdo()->lastInsertId();
            
        DB::table('financial_plans')->insert([
            'p_a_i_id' => 145,
            'source_id' => 3766,
            'value' => 1000000000,
            'functioning_percentage' => 0,
            'debt_service_percentage' => 0,
            'fund_percentage' => 0,
            'investment_percentage' => 100,
            'functioning' => 0,
            'debt_service' => 0,
            'fund' => 0,
            'investment' => 1000000000,
            'year' => 3,
            ]);
            $idPlanAniotres39 = DB::getPdo()->lastInsertId();
            
        DB::table('financial_plans')->insert([
            'p_a_i_id' => 145,
            'source_id' => 3767,
            'value' => 160017784,
            'functioning_percentage' => 0,
            'debt_service_percentage' => 0,
            'fund_percentage' => 0,
            'investment_percentage' => 100,
            'functioning' => 0,
            'debt_service' => 0,
            'fund' => 0,
            'investment' => 160017784,
            'year' => 3,
            ]);
            $idPlanAniotres40 = DB::getPdo()->lastInsertId();
            
        DB::table('financial_plans')->insert([
            'p_a_i_id' => 145,
            'source_id' => 3768,
            'value' => 2236002300,
            'functioning_percentage' => 0,
            'debt_service_percentage' => 0,
            'fund_percentage' => 0,
            'investment_percentage' => 100,
            'functioning' => 0,
            'debt_service' => 0,
            'fund' => 0,
            'investment' => 2236002300,
            'year' => 3,
            ]);
            $idPlanAniotres41 = DB::getPdo()->lastInsertId();
    }

    /**
     * Reverse the migrations.
     *
     * @return void
     */
    public function down()
    {
        //
    }
}
