...
class FixFuncFca extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        ...
        CREATE OR REPLACE FUNCTION cardinal.fca(
            ...
            p_aniorep integer,
            p_mesrep integer)
            RETURNS TABLE(father_id integer, codigo character varying, name character varying, id_plan double precision, recaudo double precision, liquidacion_fca double precision, porc_registrado double precision, code_new character varying, len_codigo double precision) 
        AS $$
        declare 
            cur_codes cursor
        ...
        --DROP TABLE temp_table_fca;
        CREATE TEMP TABLE IF NOT EXISTS temp_table_fca AS
        ...
        order by ffs.code;
            ...
            -- build the output
                ...
                up_code :=cons_fcode;
                    ...
                    update temp_table_fca set id_plan=cons_fid_plan, recaudo= cons_frecaudo, porc_registrado= cons_fundperc, liquidacion_fca= ((cons_frecaudo*cons_fundperc)/100),
                    len_codigo=len_code
                    where temp_table_fca.father_id=cons_ffather and temp_table_fca.code=cons_fcode;
                ...
                temp_recaudo := (select sum(temp_table_fca.recaudo) from temp_table_fca where temp_table_fca.code=up_code);
                temp_liquidacion := (select sum(temp_table_fca.liquidacion_fca) from temp_table_fca where temp_table_fca.code=up_code);
        ...
        RETURN QUERY 
        SELECT *
        from temp_table_fca; --where temp_table_fca.father_id=5;
        --   return table;
        DROP TABLE temp_table_fca;
        end;
        $$ language plpgsql;");
