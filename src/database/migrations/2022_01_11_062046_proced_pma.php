...
class ProcedPma extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        ...
        DB::unprepared("
        CREATE OR REPLACE FUNCTION public.pma_den(
        ...
        DB::unprepared("
        CREATE OR REPLACE FUNCTION public.pma_den_ejec(
            p_pai_id bigint,
            p_year integer)
        ...
        DB::unprepared("
        CREATE OR REPLACE FUNCTION public.pma_num(
            p_group_name character varying,
            p_pai_id bigint,
            p_year integer)
            RETURNS TABLE(id_plan bigint, category_name character varying, tipo_de_plan character varying, codigo character varying, cuenca character varying, anio1 double precision, anio2 double precision, anio3 double precision, anio4 double precision) AS $$
        ...
        --DROP TABLE temp_table;
            ...
            for reg in
                ...
                group by
                   ...
                   loop
                    register = (select count(*) from temp_table2 where temp_table2.id_plan = reg.indicator_activities_id);
                    RAISE NOTICE ' numerator: %', reg.anio1;
                    IF reg.anio1 is null THEN
                        _anio1 = 0;
                    ELSE
                        _anio1 = reg.anio1;
        ...
        DB::unprepared("
        CREATE OR REPLACE FUNCTION public.pma_num_ejec(
            p_pai_id bigint,
            p_year integer)
            RETURNS TABLE(anio1 double precision, anio2 double precision, anio3 double precision, anio4 double precision) AS $$
        declare
        ...
        DB::unprepared("
        CREATE OR REPLACE FUNCTION public.pma_weigh_ejec(
            p_pai_id bigint,
            p_year integer,
            anio integer)
            RETURNS double precision AS $$
        declare weighpma double precision; 
        begin
        
        weighpma = (select
                    indicator_activities.weighing as pma_weigh
                from activities
                inner join objectives on objectives.id = activities.objective_id
                inner join projects on projects.id = objectives.project_id
                inner join programs on programs.id = projects.program_id
